...
<a href="../es/nominal_dropout_es.html" class="btn btn-outline-primary" role="button">🌐 Lea esto en español</a>
...
PAHOabc includes the functionality to estimate the nominal dropout rate, on a per-person basis. This allows for the analysis of any pair of doses by specifying an initial dose and a final dose. For correct interpretation, the initial dose must precede the final dose in the vaccination schedule, for example: DTP1 – DTP3 or DTP1 – MCV1. Additionally, it allows users to select the level of aggregation for the results, which can be at the national level or subnational levels (ADM1 or ADM2).
...
-   `birth_cohorts`: Parameter to segment the analysis based on the year of birth of the persons in the dataframe. It is based on the `date_birth` column in the dataframe and can be a single year or a vector of years.

The `data.EIR`, `vaccine_init`, `vaccine_end` parameters are mandatory. `geo_level` and `birth_cohorts` are optional and help to segment the data for more precise analyses.
...
-   `within_ADM1`: Allows to filter within an ADM1 and display only the ADM2 data corresponding to the specified ADM1.
